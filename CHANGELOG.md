...
**问题背景**：用户反馈"生成的 xlsx 表格不应该有多个 Sheet，所有内容全部在一个 Sheet 里面显示"。v2.0.2 的 xlsx 是 7 个独立 Sheet（表头/结构/功能/失效/风险/优化/矩阵），不符合用户期望，也与 8D 报告的单 Sheet 模式不一致。

### 修复内容

完全重写 `create_excel_report` 函数，从"7 Sheet"改为"单 Sheet 8 区域"：

| 区域 | 行号区间（DFMEA 示例） | 内容 | 列数 |
|---|---|---|---|
| 标题 | Row 1-2 | DFMEA 分析表 + 产品/客户/日期 | 10 列合并 |
| 一、表头信息 | Row 4-22 | 18 项键值对（FMEA 类型/产品/客户/编制人等） | 2 列（值合并 B-E） |
| 二、结构分析 | Row 24-26 | 层级/要素/说明 | 3 列（说明合并 C-E） |
| 三、功能分析 | Row 28-30 | 层级/要素/功能 | 3 列（功能合并 C-E） |
| 四、失效分析 | Row 32-39 | 序号/FE/FM/FC | 4 列 |
| 五、风险分析 | Row 42-49 | 序号/FE/FM/PC/DC/S/O/D/AP/特殊特性 | 10 列（AP/CC/SC 高亮） |
| 六、优化措施 | Row 52-58 | 序号/FM/类型/描述/责任人/截止/状态/后S/后O/后D/后AP | 11 列（状态高亮） |
| 七、风险矩阵 | Row 61-72 | 10×10 S×O 热力图（H红/M黄/L绿） | 11 列 |
| 图例 | Row 73 | H/M/L 图例说明 | 4 列 |
| 风险统计 | Row 75-81 | 6 项统计（总数/H/M/L/CC/SC） | 4 列 |
| 八、签名栏 | Row 84-87 | 编制/审核/批准 | 4 列（姓名/签名/日期各合并 3 列） |

**关键设计**：
- 每个 Region 之间用章节标题行分隔（淡黄底深蓝字）
- 各 Region 使用合并单元格让不同列数的表格和谐共存
- AP/CC/SC/状态高亮保持（与 v2.0.2 一致）
- 行高自动计算仍然生效（_recalc_all_row_heights）
- 冻结首行（freeze_panes = "A2"），方便长表格滚动查看

### 兼容性

- 完全向后兼容：v2.0.2 的所有命令行参数和模板格式不变
- docx 输出不变：仍然是 7 个表格（v2.0.2 已改为纯表格）
- auto_fill 行为不变

### 实测验证

| 测试场景 | Sheet 数量 | 总行数 | 列数 | 章节数 |
|---|---|---|---|---|
| PFMEA + generic-fmea + auto_fill | 1 | 80 | 11 | 8 |
| DFMEA + mechanical-assembly + auto_fill | 1 | 88 | 11 | 8 |
